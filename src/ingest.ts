...
const CN_VIA_APAC_COLOS = new Set(['NRT','SIN']);  // 国内被强制调度到的海外节点

function clientCountry(request: Request): string {
  const realIp = request.headers.get('X-Real-IP');
  if (realIp) {                       // 经 LightCDN
    const cc = request.cf?.country;
    if (cc && cc !== 'APAC') return cc; // 非亚太节点 = 真实访客国（欧、美等准确）
    const colo = request.cf?.colo;
    if (colo && CN_VIA_APAC_COLOS.has(colo)) return 'CN'; // 亚太节点 = 国内走亚太线路
    return 'request.cf?.country';                      // 其余亚太 = 真实亚太访客
  }
